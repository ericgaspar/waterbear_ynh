...
/**
 * 
 * Ce WS sert � r�ceptionner un bon de commande
 * Il commence par tester le acb envoy� pour d�terminer si c'est un ISBN ou un cab document
 * dans le 1er cas, on se contente d'afficher la ligne de commande (et de l'enregistrer dans l'op�raion)
 * 
 * Si on fournit un cab document, on fait la r�ception en suivant les op�rations suivantes (utilisant de nombreux plusgins, cf ci-dessous)
 * > v�rifier s'il reste des lignes � recevoir
 * > v�rifier que le cab n'est pas utilis�
 ...
 * > Modifier la ligne de commande (modif des quantit�s / engagement / factur�s) et enregistrer
 * > idem pour la commande
 * > retourner les infos format�es
 * 
 * 
 * 
**/
...
$retour["resultat"]["texte"]=$_SESSION["operations"]["'.$ID_operation.'"]["texte"]; // par d�faut, on ne change pas le texte
...
// 2) SI EXEMPLAIRES
    ...
    // 1) R�cup�rer notice ligne de commande
    ...
    // 3) V�rifier s'il reste des lignes � recevoir
    ...
    // 4) V�rifier si le cab n'a pas d�j� �t� utilis�
    ...
    // 6) On modifie la notice exemplaire (enl�ve champ 500, ajoute cab)
    ...
    // 7) On enregistre la notice exemplaire et on r�cup�re ID_notice_exemplaire
    ...
    // 8) On r�cup�re la notice biblio
    ...
    $notice_exemplaire=get_objet_xml_by_id("exemplaire", $ID_notice_exemplaire); // On RE r�cup�re la notice exemplaire, car Sinon, le champ 000 n'est pas maj et on a l'ID de la ligne de cmde
    ...
    // 14) On r�cup�re et maj la notice de commande
    ...
    if ($champ_700=="") {
        $retour["resultat"]["message"]="Pas de champ 700 dans la notice de commande : mise � jour du budget impossible";
        $output = $json->encode($retour);
        print($output);
        die("");
    }
