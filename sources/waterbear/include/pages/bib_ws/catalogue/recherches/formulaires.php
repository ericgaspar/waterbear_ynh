...
if ($operation == "valide_formulaire") {
    ...
    // 2 bis) si bool_exporter=1, on d�sactive la limitation de la pagination
    if ($bool_exporter == 1) {
        $param_recherche["nb_notices_par_page"]=10000000;
        $param_recherche["page"]=1;
    }
    ...
    // 3) Si action panier, on supprime les param�tres de tri, page...
    if ($action_panier != "") {
        unset($param_recherche["tris"]);
        unset($param_recherche["page"]);
    ...
    // 4 bis) si exporter, on affiche tout de suite le r�sultat
    ...
    if ($bool_exporter == 1) {
        ...
        // options d'export sp�cifiques � un format de liste donn� (pour export en .pan, .xls, .txt, .htm...)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// affiche_notice_idx (rang de la notice dans une liste de r�sultat)
} elseif ($operation == "affiche_notice_idx") {
    // 1) On r�cup�re les param�tres de recherche (on ne les modifie pas)
    $param_recherche=$json->decode($_REQUEST["param_recherche"]);
    // 2) Eventuellement, on modifie (ou enrichit) les param�tres
    $tmp=applique_plugin ($GLOBALS["affiche_page"]["parametres"]["plugin_formulaire_2_recherche"], array("array_in"=>$param_recherche)); // on modifie �ventuellement les param�tres de recherche
    if ($tmp["succes"] != 1) {
    ...
    $retour=applique_plugin($GLOBALS["affiche_page"]["parametres"]["plugin_recherche"], array("param_recherche"=>$param_recherche));
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Retourne l'ID d'une notice � partir de son idx
...
// Affiche panier
    ...
    if ($type == "statique") {
        ...
        foreach ($contenu["recherchator"]["criteres"] as $critere_panier) {
            ...
            $nom_panier_comptage=$critere_panier["nom_panier_comptage"];
            $type_comptage=$critere_panier["type_comptage"];
            $type_lien=$critere_panier["type_lien"];
            ...
            $critere_formulaire["nom_panier_comptage"]=$nom_panier_comptage;
            $critere_formulaire["type_comptage"]=$type_comptage;
            $critere_formulaire["type_lien"]=$type_lien;
