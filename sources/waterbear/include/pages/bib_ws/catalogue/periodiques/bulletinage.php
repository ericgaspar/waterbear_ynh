...
$plugin_maj_dernier_no=$GLOBALS["affiche_page"]["parametres"]["plugin_maj_dernier_no"];
...
if ($type == "biblio" OR $type=="ID_notice") {
    ...
    // 1) on RAZ la liste des abos utilis�s
    ...
    // 2.a) on r�cup�re la notice de p�riodique
    if ($type == "biblio") { // ISBN
    ...
    // 2.b) On enregistre ID_ligne_commande dans l'op�ration
...
if ($type == "exemplaire") { // SI EXEMPLAIRE
    ...
    // 0) V�rifier si le cab n'a pas d�j� �t� utilis�
    ...
    // 2) On r�cup�re le n� et date du num�ro � recevoir (et � partir de �a une clef de ddbl)
    if ($bool_hs == "true") {
        ...
        $ID_dernier_no=$abo["a_id_dernier_no"];
    }
    $ddbl_fascicule=$prochain_numero."_".$prochaine_date;
    
    // 2b) On maj le dernier n° bulletiné (pour remettre en statut normal et prêtable
    if ($ID_dernier_no != "" AND $ID_dernier_no != 0 AND $ID_dernier_no != "NULL") {
        $notice_dernier_no= get_objet_xml_by_id("exemplaire", $ID_dernier_no);
        $tmp=applique_plugin($plugin_maj_dernier_no, array("notice"=>$notice_dernier_no));
        if ($tmp["succes"]==0) {
            $output = $json->encode($tmp);
            print($output);
            die("");
        }
        $notice_dernier_no2=$tmp["resultat"]["notice"];
        $tmp=applique_plugin ($plugin_notice_2_db_exemplaire, array("notice"=>$notice_dernier_no2, "ID_notice"=>$ID_dernier_no));
        if ($tmp["succes"] != 1) {
            $output = $json->encode($tmp);
            print($output);
            die("");
        }
    } // fin du si ID_dernier_no
    
    // 3) Est-ce qu'il faut cr�er une notice de fascicule ou pas ?
    ...
    if ($nb_fascicules == 1) { // Si fascicule existe d�j�
    ...
    } elseif ($nb_fascicules == 0) { // si fascicule n'existe pas, on le cr�e
    ...
    // 4) cr�ation de l'exemplaire
    // 4.a) on r�cup�re les infos sur l'exemplaire dans le champ 697 de l'abonnement
    ...
    // 4.b) on g�n�re une notice exemplaire d'apr�s le mod�le, en y int�grant les infos fournies
    ...
    // 4.c) On enregistre la notice exemplaire (et on r�cup�re un ID)
    ...
    if ($bool_hs != "true") {
        ...
        $tmp=applique_plugin($plugin_maj_abo, array("notice"=>$notice_abo, "prochaine_date"=>$date_prochain_no, "prochain_no"=>$num_prochain_no, "ID_notice_exemplaire"=>$ID_notice_exemplaire, "notice_exemplaire"=>$notice_exemplaire));
        if ($tmp["succes"] != 1) {
            $output = $json->encode($tmp);
            print($output);
            die("");
...
////////////////////////////////////////////////////////////////////////////////////////////////////
// Recherche et affichage p�riodique et liste abos

// on r�cup�re la notice
$notice_periodique=get_objet_xml_by_id("biblio", $_SESSION["operations"]["'.$ID_operation.'"]["ID_periodique"]);

//  on formate et affiche la notice de p�riodique
...
//  On r�cup�re la liste des abonnements li�s � ce p�riodique
...
// date du jour (pcque c'est gal�re en JS :/
