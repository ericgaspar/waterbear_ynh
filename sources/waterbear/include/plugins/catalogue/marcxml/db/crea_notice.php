 ...
 * @param ["liens"] => les liens sous la forme ["ID_lien"] => ID de la notice li�e, ["type_objet"] => type d'objet li�, ["type_lien"] => type de lien
 * 
 * Ce plugin enregistre dans la DB les donn�es fournies en param�tre (sous forme d'array)
 * 
 * @return
 */
function plugin_catalogue_marcxml_db_crea_notice ($parametres) {
    ...
    try {
        if ($ID != "") { // MAJ
            ...
            // on cr�e le champ 001 et on modifie la notice
            /**
    ...
    // si maj, on supprime les liens existants avant de les recr�er
    if ($ID != "")
        try {
    ...
    try {
        ...
        if (is_array($parametres["liens"])) {
            foreach ($parametres["liens"] as $lien) {
                ...
                /**
                // a) on supprime les liens de ce type d�j� existants (une seule fois par type) 
