 ...
 * @param SOIT ["chaine"] => chaine de caract�res (si chaine) et ["taille_chaine"] => longueur de la chaine (si chaine)
 * @param ["last_car"] => Position du dernier caract�re lu la fois pr�c�dente
 * @param ["plugin_split"] Plugin utilis� pour retourner la notice suivante (registre)
 * @param ["plugin_xml"] Plugin utilis� pour convertir en XML (registre)
 * @param [plugin_importe] plugin utilis� pour importer la notice XML
 ...
 * @param [tempo] : alternative � pas pour le cas o� le fichier doit �tre trait� dans sa globalit� (batch...) : attente en millisecondes entre chaque notice 
 ...
 * 
 * Ce plugin :
 * 1) r�cup�re la notice suivante dans un fichier (PA du plugin param�tr� dans le registre => $plugin_split)
 * 2) convertit la notice en marcxml ((PA du plugin param�tr� dans le registre => $plugin_convert_xml)
 * 3) r�cup�re les infos de d�doublonnage (PA du plugin param�tr� dans le registre => $plugin_get_infos_ddbl)
 * 4) d�doublonne (PA du plugin param�tr� dans le registre => $plugin_ddbl)
 * 5) r�cup�re la version affichable des 2 notices (PA du plugin param�tr� dans le registre => $plugin_affiche_notice)
 */
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 function plugin_catalogue_import_export_get_notice_pas($parametres) {
    ...
    // 1) Si fichier ou chaine de carat�res fournis, on r�cup�re la notice suivante
    $notice_brute="";
    if ($handle != "") { // Fichier
        // On r�cup�re la notice suivante
    ...
    } elseif ($chaine != "") { // cha�ne de caract�re
        // On r�cup�re la notice suivante
    ...
    // 4) � la fin, si pas de tvs_marcxml => erreur
    if ($tvs_marcxml == "") {
        $retour["succes"]=0;
        $retour["erreur"]="OK";
        return ($retour);    
    ...
    if ($ID_panier != "" AND $tmp["resultat"]["ID_notice"] != "") {
        $tvs_paniers->add_statique($ID_panier, $tmp["resultat"]["ID_notice"]);
    }

    // 6) on r�cup�re une version affichable de la notice
