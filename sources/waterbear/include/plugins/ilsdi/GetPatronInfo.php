...
function plugin_ilsdi_GetPatronInfo ($parametres) {
    ...
    // 1) on r�cup�re les infos lecteur
    $ligne_lecteur=get_objet_by_id("lecteur", $patronId);
    $lecteur["patronId"]=$patronId;
    $lecteur["lastName"]=htmlspecialchars($ligne_lecteur["a_nom"], ENT_XML1, 'UTF-8');
    $lecteur["firstName"]=htmlspecialchars($ligne_lecteur["a_prenom"], ENT_XML1, 'UTF-8');
    $lecteur["loans"]=array();
    $lecteur["holds"]=array();
    
    // 2) on r�cup�re les pr�ts
    ...
    foreach ($prets as $pret) {
        ...
        $titre=htmlspecialchars($pret["a_titre_biblio"], ENT_XML1, 'UTF-8');
        $auteur=htmlspecialchars($pret["a_auteur_biblio"], ENT_XML1, 'UTF-8');
        $bib=$pret["a_bib_exe"];
    ...
    foreach ($resas as $resa) {
        ...
        $ID_exe=$resa["a_id_exe_affecte"]; // uniquement si la r�sa a �t� affect�e
        $titre=htmlspecialchars($resa["a_titre_biblio"], ENT_XML1, 'UTF-8');
        $auteur=htmlspecialchars($resa["a_auteur_biblio"], ENT_XML1, 'UTF-8');
        ...
        array_push($lecteur["holds"], array("bibId"=>$ID_biblio, "itemId"=>$ID_biblio, "title"=>$titre, "author"=>$auteur, "locationLabel"=>$bib, "priority"=>$priority));
    }
