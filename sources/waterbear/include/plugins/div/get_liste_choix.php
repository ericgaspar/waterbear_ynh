 ...
 * 
 * Si [bool_tri_libelle]==1 le tableau sera trié par libellé (par défaut trié par code)
 * 
 * @param mixed $parametres
 * @param[nom_liste]
 * @param[restricteur]
 * @param[bool_texte]
 * @param [bool_add_vide] si vaut 1 on ajoute une valeur vide (avec intitulé tiret)
 * @param [bool_tri_libelle] : si vaut 1 le tableau sera trié par libellé
...
function plugin_div_get_liste_choix ($parametres) {
    ...
    if ($parametres["bool_add_vide"] == 1) { // si ajout d'un élément vide
        array_push($liste_tmp, array("intitule"=>"-", "valeur"=>""));
    } 
    ...
    // éventuellement tri par libellé
    if ($parametres["bool_tri_libelle"]==1) {
        usort($liste_tmp, "plugin_div_get_liste_choix_trier_par_intitule");
    }
...
function plugin_div_get_liste_choix_trier_par_intitule ($a, $b) {
    $intitule_a=$a["intitule"];
    $intitule_b=$b["intitule"];
    return(strcasecmp($intitule_a, $intitule_b));
}
