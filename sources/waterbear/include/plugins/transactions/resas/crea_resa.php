...
function plugin_transactions_resas_crea_resa ($parametres) {
    ...
    $teste=$parametres["teste"]; // si vaut 1, on ne cr�e pas la r�sa
    ...
    // 2) lister les exemplaires r�servables (en pr�t ou dispos)
    ...
    foreach ($exemplaires as $idx_exemplaire => $exemplaire) {
        ...
        $reservable=1; // bool pour dispo court et long terme => par défaut oui
        // Teste des QUOTAS
        if ($idx_exemplaire == 0) { // on ne teste les quotas que pour le 1er exemplaire
            // a) teste quotas doc (nb de r�sas faites par le lecteur pour ce type de doc)
            ...
            // b) teste quotas r�servataire (nb de lecteurs ayant r�serv� ce doc)
        ...
        if ($tmp["resultat"]["reservable"]==1) { // R�servable
            // $reservable=1; : on ne fait rien
        } elseif ($tmp["resultat"]["reservable"]==2) { // R�servable mais pas disponible (ex. en traitement)
            // $reservable=1; : on ne fait rien
            $en_pret=1;
        } else {
            $reservable=0;
    ...
    // Cre�ation de la r�servation (sauf si teste)
    if ($teste != 1) {
        // 1) cr�ation des param�tres pour les champs exemplaires (champs r�p�tables)
        $param_exes=array();
        $nouv_etat=10;
        ...
        // 2) On r�cup�re les param�tres de cr�ation de l'objet r�sa (sans les exemplaires)
        $date=date("Y-m-d");
        ...
        //3) On injecte les param�tres d'exemplaires dans les param�tres de resa
        foreach($param_exes as $param_exe) {
            array_push($param_resa["definition"], $param_exe);
        }
        
        // 4) on g�n�re l'objet marcxml correspondant � la notice r�sa
