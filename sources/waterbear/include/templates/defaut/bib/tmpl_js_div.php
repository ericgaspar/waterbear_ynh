...
// motif : chaine de caract�re contenant les �l�ments � remplacer
...
function fn_mc (e, type_obj, ID, idx) {
    ...
    // on r�cup�re le type d'�v�nement : click ou contextmenu. Peut �tre fourni sous forme de chaine (pour forcer) ou obj javascript
    var type;
    ...
    if (menu_contextuel != "") {
        ...
        menu_contextuel.destroy();
            ...
            // on en fait rien, mais je ne sais pas pourquoi, une fois sur 10, cette fonction g�n�re une erreur qui bloque si on ne fait pas un try catch :/
        }
    }
...
function date_us_2_date (chaine) {
    ...
    mois--; // on retire 1 car les mois commencent � 0
...
function recherche_rapide_submit () {
    ...
    return false; // pour enmp�cher le formulaire de valider
}

// copie le contenu d'un �l�ment dans le presse-papier
// � partir du nom (text) d'un �l�ment dom
