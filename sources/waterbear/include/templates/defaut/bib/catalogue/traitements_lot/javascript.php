...
function get_formulaire() {
    ...
    var callback = {
        ...
        success: function(oResponse) {
            ...
            var str_formulaire="<table class='formulaire_traitement_lot'>";
            ...
            for (idx in oResults['resultat']['formulaire']) {
                ...
                var type=infos_formulaire["type"];
                var str_liste_choix=infos_formulaire["liste_choix"];
                champs_formulaire.push(ID);
                
                if (type=="select") {
                    var champ="<tr><td><a href='#' alt=\""+aide+"\" title=\""+aide+"\" >"+intitule+"</a></td><td><select id='"+ID+"'/>"+str_liste_choix+"</select></td></tr> \n";
                } else {
...
function init_autocomplete(){
    ...
    oDS.responseSchema = {fields : ["nom", "id"]}; // attention BUG YUI : les donn�es ne seront pas accessibles via ["xxx"] mais [0], [1]...
