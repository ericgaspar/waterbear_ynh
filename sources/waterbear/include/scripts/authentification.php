...
// User et groupe
if ($_SESSION["system"]["user"] != "") { // 1) regarder si on est d�j� identifi�
    // on ne fait rien
...
if ($_SESSION["system"]["poste"] != "") {
    if ($_SESSION["system"]["IP"] != $_SERVER["REMOTE_ADDR"]) {
        die ("Tentative de detournement de session ! votre adresse IP (".$_SERVER["REMOTE_ADDR"].") ne correspond pas à celle que vous aviez en vous connectant (".$_SESSION["system"]["IP"].")");
    }
