...
if ($GLOBALS["tvs_global"]["conf"]["ini"]["bool_metawb"]==1) {
    ...
    if ($_REQUEST["metawb_site"] != "") {
        ...
        if ($_REQUEST["metawb_site"] == "tvs") {
            ...
            // Gestion de la redirection pour les serveurs fantomes
            if ($resultat[0]["etat"] == 3) { // si serveur fantôme...
                $chaine_get="";
                foreach ($_GET as $intitule_get => $valeur_get) {
                    if ($chaine_get != "") {
                        $chaine_get.="&";
                    }
                    $chaine_get.= urlencode($intitule_get)."=".urlencode($valeur_get);
                }
                if ($page_include=="bib") { // si bib.php
                    header("Location: ".$resultat[0]["serveur"]."/bib.php?".$chaine_get,TRUE,301);
                } else { // si bib_ws.php
                    header("Location: ".$resultat[0]["serveur"]."/bib_ws.php?".$chaine_get,TRUE,301);
                }
                die ("redirection vers le serveur".$resultat[0]["serveur"]);    
            } // fin de "si serveur fantome"
        ...
        if (count($resultat2)>0) {
            ...
            if (($date_mail > $date_reponse) OR $reponse_enquete==3) { // si on n'a jamais répondu ou si une nouvelle enquête a été envoyée depuis la dernière réponse
                if (($date_mail < $delai_relance) OR $reponse_enquete==3) { // si le mail a été envoyé il y a plus de 15 jours
                    ...
                    print ("Madame, Monsieur,<br><br>Afin de nous assurer que vous utilisez toujours Waterbear, merci de répondre au questionnaire en cliquant sur le lien ci-dessous (ça ne prendra que quelques secondes).<br><br> Merci de votre compréhension. <br><br>");
