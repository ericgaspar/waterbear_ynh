...
// r�-enregistrement de toutes les notices biblio pour cr�er la colonne a_unimarc
$sql="select * from obj_biblio_acces where a_unimarc IS NULL";

$resultat=sql_query(array("sql"=>$sql, "contexte"=>"maj22"));


while ($ligne=sql_fetch_assoc($resultat)) {
    $ID=$ligne["ID"];
    ...
    if ($tmp["succes"] != 1) {
        // die ("Erreur lors de la r�indexation de la table obj_biblio_acces : ".$tmp["erreur"]);
        print ("Erreur lors de la r�indexation de la table obj_biblio_acces : ID=$ID : ".$tmp["erreur"]."<br>\n");
    } else {
        print ("$ID OK <br>\n");
    }
}
