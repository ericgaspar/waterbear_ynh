...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// transforme un tableau contenant les �tapes d'un chemin
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Retourne des noeuds ayant une certaine valeur (et �ventuellement dont le nom est xxx)
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// retourne tous les enfants d'un noeud (non r�cursif !)
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// retourne le nombre d'enfants d'un noeud (� partir d'un ID de noeud parent)
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// retourne le chemin d'un noeud � partir de son nom et de l'ID de son parent
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// v�rifie qu'il n'y a pas de "/" dans le nom
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Cr�e un noeud (� partir de l'ID du parent et d'un nom)
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Modifie un noeud (� partir d'un ID)
// ATTENTION : n�cessite de refournir TOUS les champs (m�me ceux qui ne sont pas modifi�s) sauf parent
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Supprime un noeud (� partir d'un ID)
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// supprime toute une arborescence � partir d'un noeud
// ATTENTION on ne fait pas le log metawb ici car c'est une fonction r�cursive
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// MAJ noeud en r�cup�rant les anciennes valeurs et en modifiant ce qui est n�cessaire (n�cessite un ID)
// si [bool_force_vide]==1 alors on maj les valeurs m�me si elles sont vides (ce qui permet de supprimer une valeur dans le registre)
// si [bool_force_vide_valeur]==1 alors on maj les valeurs m�me si elles sont vides uniquement pour valeur, pas pour description
// MAJ le chemin, d�doublonne...
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Cr�e un noeud en faisant des v�rifications...
// v�rifie le nom, d�doublonne...

function niv2_create_node ($parametres) {
	$retour=1;
  	$parametres["nom"]=trim($parametres["nom"]);
  	// On v�rifie que ID_parent est bien numerique
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// retourne une branche du registre en v�rifiant si elle n'est pas d�j� dans $_SESSION et en la mettant dans $_SESSION
// le param�tre $noeud est optionnel : contient et ID et valeur du dernier noeud du chemin pour �viter d'avoir � faire 2 fois la recherche (r�cursivit�)
...
function registre_get_branche ($chemin, $noeud) {
    ...
    // 1) on r�cup�re le noeud correspondant au chemin dans $_SESSION
    ...
    // 4) On r�cup�re les infos du dernier �l�ment s'ils ne sont pas fournis (c'est � dire pour la 1ere fonction mais pas pour les r�cursives)
    if ($noeud=="") {
    ...
    // 8) on indique que cette branche est compl�te
    $tmp["_COMPLET"]=1;
    
    // 9) on maj $_SESSION (uniquement fonction de base, ps les r�cursives)
    //    et on renvoie la branche (expurg�e) 
    if ($noeud == "") {
        $_SESSION["registre"]=maj_valeur_array($_SESSION["registre"], $chemin, $tmp);
        return($this->registre_expurge_retour($tmp));
        //return($tmp);
    } else { // sinon, on renvoie la branche NON expurg�e
        return ($tmp);
    }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//// R�cup�re juste les enfants d'un noeud (pas toute la branche)...
/// en tenant compte de $_SESSION
// valeur de [_COMPLET] : 1 -> la branche est compl�te
...
function registre_get_enfants ($chemin) {
    ...
    //   si que des valeurs alors la branche m�re est compl�te
    $bool_complet=1;
    foreach ($enfants as $enfant) {
        if (isset($tmp[$enfant["nom"]])) { // Si l'enfant est d�j� dans $_SESSION...
            if ($tmp[$enfant["nom"]]["_COMPLET"]!=1) {
                $bool_complet=0;
            } 
        } else { // Si non pr�sent dans $_SESSION...
            $nb_petits_enfants=$this->get_nb_enfants($enfant["ID"]);
            if ($nb_petits_enfants == 0) { // branche compl�te (valeur)
                $tmp[$enfant["nom"]]=$enfant["valeur"];
            } else { // branche non compl�te (array)
                $bool_complet=0;
    ...
    // 8) Si tous les enfants sont complets, la branche est compl�te
    if ($bool_complet == 1) {
        $tmp["_COMPLET"]=1;
    } else {
        $tmp["_COMPLET"]=2;
    ...
    // 9) on maj $_SESSION et on renvoie la branche (expurg�e) 
    $_SESSION["registre"]=maj_valeur_array($_SESSION["registre"], $chemin, $tmp);
    return($this->registre_expurge_retour($tmp));
    //return($tmp);
}
...
function get_branche_compilation ($chemin, $bool_include) {
    ...
    // Si chemin non d�fini, on renvoie faux
    if ($tmp === false) {
    ...
    // si chemin d�fini et pas array, OK
    if (! is_array($tmp)) {
...
function registre_expurge_retour ($tableau) {
    ...
    //return ($tableau); // ----> fonction d�sactiv�e (pour gagner en performances ??)
  	if (! is_array($tableau)) {
	    return($tableau);
	}
	$retour=array();
	foreach ($tableau as $idx => $element) {
	  	if ($idx !== "_COMPLET") { // ATTENTION ne pas mettre != MAIS !== sinon �a plante quand $idx == 0. Ne me demandez pas pourquoi, je n'y comprends rien !
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Cette fonction cr�e un nouveau noeud � partir d'un chemin et d'une valeur
// Elle peut cr�er r�cursivement son chemin si celui-ci n'existe pas
// si le noeud existe d�j�, il est mis � jour
// la fonction est r�cursive (si le chemin n'est pas trouv� imm�diatement. Au d�but, $depart=array() puis �chaque fois contient plus de noeuds non trouv�s
function create_node_chemin ($depart, $chemin, $valeur, $description) {
  	if ($chemin=="") { // si on est au bout
	    $ddbl_noeud=array("ID"=>0);
	} else {
  		$ddbl_noeud=$this->get_node_by_chemin($chemin); // On essaye de r�cup�rer le noeud
  	}
  	if ($ddbl_noeud != false) { // s'il existe, MAJ
  	  	// On cr�e les noeuds interm�diaires si n�cessaire (cas o� on utilise la r�cursivit�)
		...
		// On r�cup�re le dernier noeud et on le maj
	...
	} else { // sinon, on appelle r�cursivement la fonction
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Cette fonction copie une branche � un autre endroit de l'arbre
...
//              $destination (ID) et $modele (ID) : le modele et la destination  (utilis�s ensuite par la r�cursivit�)
//              $copie_contenu : si=1 on ne copie pas le noeud acine du mod�le, mais seulement ses enfants (toujours � 0 en r�cursivit�)
function copy_branche ($parametres) {
  	extract($parametres);
  	if (isset($modele_str)) { // si on a fourni des chemins, on r�cup�re les noeuds
		...
		$modele=$modele2["ID"];
	  	...
	  	$copie_contenu=1; // en r�cursivit�, on ne copie toujours QUE les enfants
...
// la f� est appel�e la premi�re fois avec un chemin. Les it�rations suivantes, c'est le noeud lui-m�me qui est pass�
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Cette fonction exporte une branche qui pourra ensuite �tre import�e par la fonction niv2_create_node()
// la f� est appel�e la premi�re fois avec un chemin. Les it�rations suivantes, c'est le noeud lui-m�me qui est pass�
// si $bool_delete_node==1 on rajoutera une ligne posur supprimer le noeud (avant de le recr�er)
// retourne sous forme d'array (� convertir en json pour pouvoir �tre trait�)
...
// remonte de parent en parent jusqu'� ce qu'il trouve un tag (autorisant ou refusant l'export). Si rien trouv� on autorise
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Rep�re et supprime les �ventuels doublons de registre qui peuvent survenir quans on a malencontreusement lanc� 2 mises � jour simultan�ment
// si $parametres["bool_test"]==1 les noeuds ne sont pas supprim�s (simple test)
...
function ddbl_registre ($parametres) {
    ...
    foreach ($doublons as $doublon) {
        ...
        // On analyse quels ID sont vides pour �tre supprim�s
        ...
        // Contr�le de coh�rence et suppression des doublons
        if ($bool_garde==1 AND $bool_suppr==1) {
            ...
            array_pop($ids_a_supprimer); // on conserve un noeud m�me vide
            foreach ($ids_a_supprimer as $id_a_supprimer) {
                if ($bool_test != 1) {
                    $this->delete_node($id_a_supprimer);
                }
