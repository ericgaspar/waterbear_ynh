...
/**
 * recherche_simple
 * Cette classe g�re en fait toutes les recherches (m�me les complexes (jointure...)
 * 
 * 
 * 
 * param�tres de la fonction init():
 * [type_objet] => type d'objet � rechercher
 * [criteres][0,1,2...][booleen (AND, OR...) | type_recherche (str_commence, str_contient...) | intitule_critere (a_titre, a_auteur...) | valeur_critere (la chaine � rechercher) | plugin_formate_critere (un plugin pour formater le crit�re par ex. transformer "an" en "2013")]
 * [tris][0,1,2...] => les colonnes � utiliser pour trier
 * [page] => page � afficher
 * [nb_notices_par page] 
 * [format_resultat] => str_sql : la requete SQL | donnees : une array des r�sultat (en tableau) contenant ou non la notice xml | formate : tableau ou chaine format�s
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// maj les param�tres de recherche
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// genere_sql_criteres
// g�n�re une chaine de caract�res SQL pour les crit�res
...
function genere_sql_criteres ($criteres="") {
    ...
    foreach ($criteres as $critere) {
        $critere=secure_sql($critere); // on s�curise les infos
        ...
        $schema_jointure=$critere["schema_jointure"]; // param�tres d'une sous-requ�te de jointure, dans laquelle valeur_critere est inject� sous forme d'une variable incluse        
        ...
        // TODO g�rer ici les crit�res ne n�cessitant pas de valeur (est vide, est non vide...)
        
        // Si pas de valeur, on ignore le crit�re (sauf recherches sp�cifiques du type est nul ou n'est pas nul)
        if ($valeur_critere === "") {
        ...
        // si jointure fournie via un formulaire de recherche, on remplace valeur_critere par schema_jointure (dans lequel valeur_critere a �t� au pr�alable inject� via une variable incluse)
        if ($schema_jointure != "") {
        ...
        if ($type_recherche == "str_commence") {
            ...
            $valeur_critere=str_replace('**', '"', $valeur_critere); // pour des raisons de caract�res sp�ciaux en remplace les " par ** puis on fait l'inverse :/
            $valeur_critere2 = $this->json->decode($valeur_critere);
            $phrase=$this->genere_sql_criteres($valeur_critere2);

        } else { // autres op�rateurs 'simples' (=, >, <, >=, <=, != ...)
...
function type_recherche_2_operateur ($type_recherche) {
    $types=array("str_egal" =>"=", "int_egal" =>"=", "egal" =>"=","inf"=>"<", "inf_egal"=>"<=", "sup"=>">", "sup_egal"=>">=", "is_null"=>"=", "is_not_null"=>"!=", "non_egal"=>"!=");
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// genere_sql_tri
// g�n�re une chaine de caract�res SQL pour le tri
...
// g�n�re une chaine de caract�res SQL pour la pagination
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// genere_sql
// g�n�re une chaine de caract�res SQL pour la requete
...
// g�n�re une chaine de caract�res SQL pour la requete. Comme requete normale, mais ID au lieu de * et pas de tri ni pagination
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// genere_sql_count
// g�n�re une chaine de caract�res SQL pour la requete de type count(*)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// genere_sql_sum
// g�n�re une chaine de caract�res SQL pour la requete de type sum(xxx)
...
function formate_resultat () {
    ...
    // Format Donn�es : Tableau pouvant OU NON contenir la notice en XML
    // !!! NON ne peut pas contenir la notice XML pour l'instant (� modifier ???)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// ajoute des '*' � la fin des mots pour une recherche de type "contient mots commen�ant par..."
...
// Si bool_descente vaut == 0, on retourne juste le SQL inclus, c'est � dire sans faire la descente qui permet v�ritabelment la jointure
//  => utilis� pour le comptage
...
function panier_lien_2_critere($chemin, $criteres, $type_obj_lien, $type_lien, $sens_lien, $bool_descente) {
    ...
    $sql_type_lien=""; // opt ** en cas de restriction � un type particulier de lien
    $sql_in="";
    
    
    // 1) on g�n�re le sql inclus (sp�cifique au panier) ==> on obtient par ex. des ID exemplaires
    if ($type == "statique") {
        $sql_in=$contenu;
    } elseif ($type == "dynamique") {
        $tmp_recherche=new recherche_simple();
    ...
    // 2) on g�n�re le sql g�n�ral ==> pour obtenir par ex. des ID biblio
    if ($sens_lien == "explicite") {
        // 2.a on d�termine la table de jointure
        $table_jointure="obj_".$this->type_objet."_liens";
        
        // 2.b on g�n�re le SQL li� au type de lien (701, 702...)
        if ($type_lien != "") {
        ...
        // 2.a on d�termine la able de jointure
...
function notice_str_2_xml ($ligne) {
    ...
    // avant de le faire on supprime ceux qui existaient d�j� (le cas �ch�ant)
