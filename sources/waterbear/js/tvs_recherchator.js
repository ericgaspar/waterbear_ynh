...
this.ID=0; // id des �l�ments dans le conteneur
...
this.plugin_get_id; // plugin de formatage pour r�cup�rer juste l'id d'une notice
this.type_2_grille;
...
this.init_formulaire = function () {
    // 1) on d�termine le nom des diff�rents <div> en fonction du pr�fixe
    ...
    // 3) Init le formulaire d�faut
    this.init_formulaire_defaut();
    
    // 4) �ventuellement on lance la recherche si vlaidation_auto == 1
...
this.add_champ_recherche = function (param) {
    ...
    champ.post_genere_champ(); // � appeler une fois que le DOM a �t� maj
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// Afficher la liste des champs � ins�rer
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// G�n�re une array � partir du formulaire
...
this.formulaire_2_array = function () {
    ...
    for (idx in liste) {
        ...
        element.valeur_critere=element.valeur_critere.replace("&", " ");
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// G�n�re une cha�ne json � partir du formulaire
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// Exporte la liste dans une nouvelle fen�tre
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// Valide le formulaire pour ajouter les notices trouv�es au panier en cours
    ...
    var this_recherchator = this;
        ...
        success: function(oResponse) {
            ...
            if (oResults.succes != 1) {
                ...
                if (bool_maj_barres == true) { // si recherche directe (par oppos� � tri ou format...) on maj les barres
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// lance requ�te pour affichage du total
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// Valide le formulaire pour la g�olocalisation
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// Affiche une liste de r�sultats
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// Affiche une notice par son idx (c'est � dire sa position dans la liste des r�sultats)
    ...
    var this_recherchator = this;
        ...
        success: function(oResponse) {
            ...
            if (oResults.succes != 1) {
                ...
                document.getElementById(this_recherchator.nom_div_notice_en_cours).innerHTML=this_recherchator.idx_notice_en_cours+1;
                this_recherchator.tabView.selectTab(this_recherchator.no_onglet_notice);
                this_recherchator.get_ID_by_idx(idx); // pour mettre � jour this.ID_notice_en_cours
			}
        },
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// R�cup�re l'ID d'une notice � partir de son idx et met � jour this.ID_notice_en_cours
...
this.affiche_pagination = function (nb_notices, nb_pages) {
    ...
    var html="<table id='table_barre_liste'><tr>"; // d�but du tableau
    html+="<td id='col_nb_notices_liste'><span id='nb_notices_liste'>"+nb_notices+" notices</span></td>"; // nb de notices
    html+="<td id='col_pagination_liste'><table width='100%'><tr>"; // d�but cellule pagination
        html+="<td><img src='"+img_debut+"' onclick='"+this.nom_js+".go_page(1)'/></td>"; // icone d�but
    ...
    html+="<td with='35%'>trier par : <select onChange='"+this.nom_js+".valide_formulaire(false)' id='"+this.nom_div_tri+"'>"; // d�but tri
    var selected="";
    ...
    html+="<td with='30%'>Format : <select onChange='"+this.nom_js+".valide_formulaire(false)' id='"+this.nom_div_format_liste+"'>"; // d�but Format
...
this.affiche_pagination_notice = function (nb_notices) {
    ...
    var img_suivant=add_skin("IMG/icones/control_fastforward_blue.png");
    var img_fin=add_skin("IMG/icones/control_end_blue.png");
    var html="<table id='table_barre_notice'><tr>"; // d�but du tableau
    html+="<td id='col_pagination_notice'><table width='50%'><tr>"; // d�but cellule pagination
        html+="<td><img src='"+img_debut+"' onclick='"+this.nom_js+".go_notice(1)'/></td>"; // icone d�but
    ...
    html+="<td with='50%'>Format : <select onChange='"+this.nom_js+".affiche_notice_idx("+this.nom_js+".idx_notice_en_cours)' id='"+this.nom_div_format_notice+"'>"; // d�but Format
    var selected="";
...
this.go_page = function (indicateur) {
    if (indicateur==1) { // d�but
        this.page=1;
    } else if (indicateur == 2) { // pr�c�dent
        if (this.page<=1) {
            return (0);
        }
        this.page--;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// R�cup�rer le format pour affichage de la notice d�taill�e
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// R�pond � un appel d'une autre page
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// maj le nom d'un panier quand on a ouvert une nouvelle fen�tre
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// Rebondir � partir de la notice courante (idem menu contextuel)
