function tvs_champ_recherche (parametres) {
    // Les icones peuvent soit ex�cuter des fonctions globales
    // ou des m�thodes propres au champ auquel elles sont associ�es
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // fonction par d�faut mais doit �tre surcharg�e la plupart du temps
    ...
    this.valide_champ = function () {
        ...
        // si schema_jointure, on remplace @valeur_critere et @type_recherche par les variables du m�me nom
    ...
    this.set_champ = function () {
        ...
        // g�n�re un champ de recherche sp�cifique (textbox, autocomplete, select...) 
    }
    
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    this.formate_jointure = function (schema_jointure, valeur_critere, type_recherche) {
        var retour_jointure=new Object();
        for (id in schema_jointure) {
            if (typeof(schema_jointure[id])=="object") {
                //schema_jointure[id]=this.formate_jointure(schema_jointure[id], valeur_critere, type_recherche);
                retour_jointure[id]=this.formate_jointure(schema_jointure[id], valeur_critere, type_recherche);
            } else {
                //schema_jointure[id]=String(schema_jointure[id]=schema_jointure[id]);
                ...
                retour_jointure[id]=String(schema_jointure[id]=schema_jointure[id]);
                retour_jointure[id]=retour_jointure[id].replace("@valeur_critere", valeur_critere);
                retour_jointure[id]=retour_jointure[id].replace("@type_recherche", type_recherche);
            }
        }
        return(retour_jointure);
    }
    
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    this.post_genere_champ = function () {
        // A SURCHARGER DANS LES ENFANTS (optionnel)
        // fonction appel�e UNE FOIS que le DOM a �t� mis � jour avec les donn�es retourn�es par genere_champ()
    }
...
function tvs_champ_autocomplete (parametres) {
    ...
    // Param�tres sp�cifiques � autocomplete
    ...
    this.input; // �l�ment <input> de l'autocomplete (o� on saisit le texte)
...
function tvs_champ_panier_lien (parametres) { // h�rite de tvs_champ_autocomplete
    // H�ritage
    ...
    this.valide_champ = function () {
        ...
        retour.valeur_critere=this.get_valeur();
        retour.type_obj_lien=this.type_obj_lien;
        retour.type_lien=this.get_type_recherche(); // on utilise la m�me m�thode, mais en fait c'est le type de lien qu'on r�cup�re, pas le type de recherche
        retour.sens_lien=this.sens_lien;
        retour.autoplugin=this.autoplugin;
        return (retour);
    }
    ...
    this.set_types_liens = function () {
        ...
        for (bidon in this.liste_types_liens) {
            ...
            if (String(valeur) == String(this.type_lien)) {
                selected=" selected ";
            }
            
            str+="<option  value='"+valeur+"'"+selected+">"+intitule+"</option>";
        }
...
function tvs_champ_comptage (parametres) { // h�rite de tvs_champ_panier_lien
    // H�ritage
    this.parent = tvs_champ_panier_lien;
    this.parent(parametres);
    
    this.nom_panier=parametres["nom_panier_comptage"];
    this.type_recherche=parametres["type_comptage"]; // cette variable est utilisée par set_types_recherches() pour générer le champ
